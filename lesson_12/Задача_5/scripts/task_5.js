...
// -----шукаємо ім'я-----
function includesName(arrNameSort, searchName) {
  let start = 0
  let end = arrNameSort.length - 1

  while (start <= end) {
    const middle = Math.floor((start + end) / 2)
    if (arrNameSort[middle] === searchName) return middle
    if (arrNameSort[middle] < searchName) start = middle + 1
    if (arrNameSort[middle] > searchName) end = middle - 1
  }
  return -1
}

const index = includesName(arrNameSort, searchName)

if(index === -1) document.write(`В даному масиві не має шуканого елементу`)
else document.write(`У масиві шуканий елемент ${searchName} знаходиться під індексом ${index}<br>`)
